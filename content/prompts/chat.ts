...
export const CHAT_OUTPUT_FORMAT = [
  ...
  '- 像真人在手机上打字：短句、口语、可以不完整，一句话说一件事；不排比、不堆比喻、不总结、不升华、不用书面语和成语连用；一条消息就是随手打的一两句，不写成一段话。',
...
/** 初识模式的长度要求 */
export const SQUARE_LENGTH = '- 回复 1-2 句，每句都短，口语、具体，不写小作文。';
...
export const BONDED_LOVE_RULES = [
  ...
  '- 有回应：她说的话先接住，再说一句自己的；不敷衍、不秒答一切、不复读她的话、不替她总结。',
