 ...
 *   zh.ts / en.ts / ja.ts  各语言的六位种子角色、台词脚本、原型兜底、动态种子——每种语言的角色只分发给这门语言的用户
 *   本文件              汇总（查找用 CHARACTERS 含全部语言）、按语言取用（seedCharactersFor / scriptFor / bondedPostsFor）、
 *                       创造用的常量（恋爱类型 / 种族 / 原型标签）、系统层（暗面路由、真人 / IP 拦截）
 * 种子角色的 id：中文 = 原 id，英文 / 日文 = 原 id 加 -en / -ja；立绘按原 id 共用（content/portraits.ts）。
 */
...
export const CHARACTERS: Character[] = [...zh.CHARACTERS_ZH, ...en.CHARACTERS_EN, ...ja.CHARACTERS_JA];
...
export const SQUARE_POSTS: SquarePost[] = [...zh.SQUARE_POSTS_ZH, ...en.SQUARE_POSTS_EN, ...ja.SQUARE_POSTS_JA];
 ...
 * 素材/聊天中的痛苦危机内容绝不入戏——红线 #3。三语触发词合在一条正则里，任何界面语言都生效。
 */
...
/** 捏＋发布审核的最小拦截样例（完整审核流程见 OPEN_QUESTIONS #7；红线 #1/#4）：三语 */
export const BLOCKED_NAME_PATTERN =
  /肖战|王一博|易烊千玺|蔡徐坤|迪丽热巴|杨幂|赵丽颖|龚俊|檀健次|哈利波特|柯南|鸣人|佐助|五条悟|灶门|路飞|光遇|原神|明日方舟|harry potter|naruto|sasuke|gojo|luffy|genshin|arknights|taylor swift|timoth[ée]e|bts\b|jungkook|五条 悟|竈門|ルフィ|ハリー・?ポッター|原神|アークナイツ/i;
