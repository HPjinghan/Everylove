...
## D-078 · 2026-09-02 · 提示文案去机制化：界面提示不解释系统设计，交互模式交给用户自己探索（Harper 提出）

- **背景**：Harper「集中清理一下所有的提示文案，不要把系统设计放在里面，比如『他会 xxx』这样的句子。集体的交互模式更多的交给用户自己探索」。此前大量 hint / 空状态 / 弹层副标题在替系统做说明：「3 天不聊 TA 会忘记你」「心动满了 TA 会想和你确定关系」「加好友之后 TA 会搬进你的 Message」「左滑略过 · 右滑心动」「羁绊 LV3 起每亲近一级解锁一条」「动漫走蒸汽机模型」等。
- **决策（文案规范，自此生效）**：
  1. **提示文案只描述内容或情绪，不描述机制**：不写「TA 会…」「满 100 就…」「N 天不聊就…」「LV 几解锁…」「进入共享池 / 走某某模型」这类系统行为与数值；不写导航指路（「设置 → 订阅计划」「到『外出』里赴约」「去『交友』滑一滑」）。
  2. **交互方式不预告**：滑卡方向、点卡即配对、长按删除、停顿即发送等一律不写字面说明，让用户上手摸索；印章、按钮、动效本身就是提示。
  3. **保留的三类**：红线与政策告知（不收真人照片、未成年不开放恋爱、发布即确认成年）；隐私与数据说明（位置只存本机、按最高敏感级）；商业信息（Pro / Max 加槽、试装不扣费）。
  4. **表单字段的 hint 只定义「填什么」**（「TA 挂在嘴边的话」「TA 藏着的事，一行一条、浅的在前」），不说系统拿它做什么；选项自带的含义短语（主动联系强度「常常先来找你 / 多半等你先开口」、节奏「一眼就沦陷 / 慢热」）是选项定义，保留。
  5. **叙事化系统消息保留**（「你们交换了联系方式 · 他开始叫你「x」」「羁绊升级 · 常常想起」「你们一起去了海边」），但去掉其中的指路后缀；试聊过期改叙事口吻「隔了太久，TA 已经不记得你了。」
- **本次改动一览**：onboarding 四个取向选项去副标题、去「不用你主动。选好之后，等他来。」与两条尾注；桌面揭幕三张卡改为一句话、去「TA 的一切都会发生在这里」；交友去「3 天不聊会过期」「左滑略过 · 右滑心动」「点一下卡片 = 心动」「随时可以换着看」，配对成功副标题改「TA 也心动了」；试聊去横幅说明、CTA 去「羁绊 LV1 ·」前缀与「会搬进 Message」；外出去「直接遇到陌生人」「聊得来 TA 会想留下你的联系方式」与两条指路空状态、「晚上八点」残留；通讯录 / Message / 电话 / 相册 / X 空状态只留一句；日历去「长按日程可删除」；创造 12 处 hint 收成字段定义、画风耗时去模型名；缔结页去「TA 会搬进你的消息里」、槽满提示只留商业信息；设置去「开得越多 TA 越懂你」「TA 眼中的你…」「即刻生效」「缔结才占槽」；我的身份去「稳定记住」「换一种活法」；天气页去假天气说明；TA 主页秘密去「关系越近，看得越深」；通话状态「听到了，停一下就发出去」→「听到了」；语音占位「轻点查看文字 · 语音占位」→「看文字」；Message 里开门遗留的「今晚 20:00 来找你」死代码删除。
- **i18n**：`lib/i18n.ts` 删 66 组失效键（含两条 D-077 前遗留的孤儿键），补 56 组新键 en/ja；顺手补齐此前缺译的 onboarding / 揭幕 / App 名（相册 / 通讯录 / 电话 / 创造 / 外出）。
- **影响文件**：`app/onboarding.tsx`、`app/index.tsx`、`app/apps/{dating,outing,contacts,messages,phone,album,moments,calendar,create,settings,identity}.tsx`、`app/chat/[characterId].tsx`、`app/outing/[placeId].tsx`、`app/adopt/[characterId].tsx`、`app/bond/[bondId].tsx`、`app/call/[characterId].tsx`、`app/weather.tsx`、`components/chat-thread.tsx`、`store/app-store.ts`、`lib/i18n.ts`。

## D-079 · 2026-09-02 · 外出优化：会话续上（一小时冷却）、照片即入相册不跨会话、对话约定进日程（迟到/爽约 TA 知道）、外出对话进记忆（Harper 提出）

- **背景**：Harper 四条：① 没点「结束外出」下次进来应该还是 TA，一小时没说话才刷新；② 合影不要跨会话发送，留在原来的会话里，用户没等到就直接存相册并 toast；③ Message 里聊到「什么时间约好在哪见面」要记进日程，到时去 TA 就在对应场景等，迟到 TA 也知道；④ 赴约内容结束外出时总结进记忆，爽约也是。
- **决策**：
  1. **会话续上与冷却**：`OutingSession.lastActiveAt`（有人说话才刷新，照片洗好贴进来不算）；进入同一地点：一小时内说过话就续上，超过一小时（`OUTING_IDLE_MS`）先体面结束上一场再开新的一场；换地点照旧先结束。生命周期编排收进 `lib/outing.ts`（`enterPlace` / `finishOuting`），场景页不再直接调 store 的 startOuting/endOuting。外出首页的地点卡标「和{name}在一起」表示 TA 还在那儿。
  2. **照片不跨会话**：新增 `store.album: AlbumShot[]`——按下快门、图洗好即 `addAlbumShot`（资产在相册，不再并入 bond.messages；相册页兜底仍汇集 D-079 之前存档里的会话图片）。生图在 `shootPhoto` 里独立于场景页运行：洗好时这一场还在就贴进现场；她已不在场景页（`setSceneVisible`）就只进相册并 `showToast('照片洗好了，已存进相册')`。新增全局轻提示 `components/toast.tsx`（`ToastHost` 挂根布局）。`endOuting` 只留系统记录（赴约「你们在××见了面」/ 偶遇「你们一起去了××」）。
  3. **对话里的约定 → 日程**：`OutingPlan` 加 `at`（时间）与 `source`（manual/chat）。TA 每回完一轮（Message 与电话挂断后）跑 `detectAppointment`：先关键词粗筛（地点味 + 时间味，中/英/日），命中才调一次模型（prompts §7 `APPOINTMENT_EXTRACT_SYSTEM`，只在**双方明确约定、地点能对应六个地点之一、时间具体到日期**时输出 `{placeId, at}`；时段无钟点按 早上 09:00 / 上午 10:00 / 中午 12:00 / 下午 15:00 / 傍晚 17:30 / 晚上 19:00 / 深夜 21:30；改期按最新、取消输出 cancel）。命中即 `addOutingPlan(…, { at, source: 'chat' })`——羁绊会话留「你们约好了 9月3日 周三 15:00 在街角咖啡馆见面」，日历关系层落一条「15:00 和 TA 约在街角咖啡馆」。同一角色同地点 ±5 分钟内不重复记；同角色只保留最新一条（原规则）。
  4. **赴约窗口与迟到**（`lib/appointments.ts`，纯函数）：有时间的约定只在 **前 2 小时 ～ 后 3 小时** 内算赴约（`planIsOpen`）；窗口内进对应地点 = 赴约，session 记 `planAt` / `lateMinutes`，prompt「此刻」段按 准时（±10 分钟）/ 早到 / 迟到 三种写法（迟到：TA 等了这么久，按性格反应，不愧疚绑架），开场白迟到专版 `OUTING_OPENERS.dateLate`；没到窗口时 TA 不会「恰好」在该地点被偶遇到。没时间的约定（外出页手动约）行为不变。
  5. **爽约**：启动/回前台 `checkMissedPlans`——过了窗口没去的约定撤掉，羁绊会话留「你错过了 … 的约」，直接写一条记忆 `[节点] … 她爽约了 …`（`addMemoryFact`，不经模型），并让 TA 主动说一两句（引擎 bonded 模式，`missedDateUserLine` 只作本轮 user 文本；AI 不可用则沉默，记忆里已记着）。
...
## D-081 · 2026-09-02 · 会话「+」面板：表情下线，改为 外出邀请 / 查 TA 的手机 / 红包 / 位置；卡片消息；录音条不再换行（Harper 提出）

- **背景**：Harper「聊天界面的右下角，表情包删掉，改成一个 +，点开后可以选择：1 发起外出邀请 2 查他的手机 3 发红包 4 发送位置；另外现在发送语音的时候换行超框」。
- **决策**：
  1. **「+」面板**：`ChatThread` 去掉表情快捷面板（EMOJIS 与表情按钮删除；系统键盘自带 emoji），新增 `extras` prop——调用方给项目，有才显示「+」（羁绊会话给四项；试聊与外出不给，无「+」）。面板为微信式四宫格（图标方块 + 短标签），点项即收起并执行。
  2. **卡片消息**：`MessageKind` 加 `'card'`，`ChatMessage.card: ChatCard { type: invite | redpacket | location, title, subtitle, amount, placeId, claimed }`。邀请/位置为浅色卡（kicker + 标题 + 副标题），红包整个气泡染红、金额大字、「等 TA 拆开 / 已领取」。进模型上下文走 `messageContextText`（「（她发来一张外出邀请：约你去××）」「（她给你发了一个 ¥52.00 的红包，留言「…」）」「（她发来了自己的位置：…）」）；Message 列表预览显示卡片标题。每张卡片计一次开口（+XP）。
  3. **外出邀请**：选六个地点之一 → 卡片上屏 → 立即 `addOutingPlan(source: 'manual')`（TA 一定答应，会话留「你们约好了去××见面」）→ TA 用引擎回一句（提示语让 TA 答应、可顺口定时间；定了时间由 D-079 的约定识别接住升级成带时间的约）。
  4. **红包**：金额预设 5.20 / 13.14 / 52 / 99 / 520 或手填（0 < x ≤ 9999），留言默认「给你的」；试装为游戏币不扣费。TA 回完话卡片标「已领取」（`patchMessage`）。
  5. **位置**：天气页设过的城市（「当前位置」）或这个世界的六个地点之一 → 「我在××」卡片 → TA 回应。
...
## D-086 · 2026-09-04 · 底座与插槽：参考 Cordis / dsh 可用的部分，建 core/（一个 Core、七个插槽、一条管线）与 features/（一个玩法一个文件）；prompt 快照 + 回合管线测试作安全网（Harper 提出）

- **背景**：Harper「我想学着改一下底层架构，让后续的开发更舒适」「我要的就是一个可插拔的结构和一个底座，这样以后可以任意引入新东西」「参考 Cordis 可用的部分，综合出方案」。先读了 deepseek-harness（编程智能体壳，Node、27.5 万行、96 个包，整体不适用），取其底座 Cordis 与 dsh 架构文档里对本 App 有用的部分。
- **架构缺点（重构前）**：① 「她说一句、TA 回一句」在 5 处各写一遍（羁绊页 / 试聊页 / 外出页 / lib/call / lib/chat），细节互相漂移（语音回复只在羁绊页、解锁暗号在 call.ts 用非 i18n 文案、外出不剥标记）；② 引擎里 Anthropic / 千帆的 fetch 代码在 generateReply 与 completeText 各抄一份，四段几乎一样；③ 四个 build 函数手工拼系统 prompt，共享块在 6 个 builder 里复制，加一条跨模式规则要改三处；④ 回复暗号在三个文件里硬编码；⑤ 卡片种类在 messageContextText / CardBody / 会话页提示语 / 面板四处硬编码，发卡片的提示语是界面里临时拼的字符串（违反 D-017）；⑥ 根布局手写 5 个启动任务 + 5 个回前台任务；⑦ 零测试；⑧ 界面直接 import 引擎 / 记忆 / 约定识别；⑨ `process.env.EXPO_PUBLIC_*` 散在 5 个模块里。
- **决策**：
  1. **从 Cordis / dsh 借的**：插件 = 往共享注册表里注册；注册即效果、可撤销（`register()` 返回撤销函数、同 key 覆盖）；类型化钩子只留 emit / waterfall 两种并挂在具名钩子点上；系统 prompt 按分段 + 具名顺序槽装配；能力接缝 = 定义 / 实现 / 消费者三件套；「模型看得见的，日志里必须有」。**不借**：`ctx.xxx` 动态挂载与声明合并、inject 依赖排序、通用事件总线、profile / bundle 分层配置、会话日志投影（多产品形态与多租户的需求，本 App 没有）。
  2. **`core/`（底座，不含任何玩法）**：`registry`（注册表）/ `hooks`（emit / waterfall）/ `config`（所有 EXPO_PUBLIC_* 只在这里读，字面量访问）/ `providers`（`ChatProvider` 接缝 + 取路 直连 > 代理 > 抛错）/ `prompt`（`PromptSection { name, modes, order, lines }`、`ORDER` 顺序槽、`assembleSystemPrompt`；装配模式 = 四种对话 + `note`）/ `modes`（`ConversationMode`：context / append / creditUserTurn / patch / maxBubbles / stripStage）/ `markers`（回复暗号 → `reply.flags` → apply）/ `cards`（卡片种类：contextText + render + bubbleColor）/ `jobs`（后台任务，launch / foreground）/ `turn`（唯一的回合管线：`runTurn` / `sendText` / `sendCard` / `respond` + `turnHooks.bubble` / `turnHooks.after`）。没启动就调用抛「底座未启动」（dsh：配置错误要响）。
  3. **`features/`（一个玩法一个文件，只往插槽注册）**：providers（anthropic / qianfan）、prompts（基础分段）、modes（square / bonded / call / outing）、invite / red-packet / location / phone-peek（各自的卡片 + 暗号 + prompt 分段 + send 函数）、voice-reply（bubble 钩子）、memory / appointment / adoption（after 钩子；领养触发器仍是产品触发器，D-008 不变）、schedulers（六个后台任务）；`features/index.ts` 是启动清单，`app/_layout.tsx` 顶部 import 一次。关掉一个玩法 = 注释一行。
  4. **管线纪律**：所有会话（羁绊页 / 试聊页 / 外出页 / 通话 / 查手机 / 爽约提醒 / TA 写记事本）走 `runTurn`，只传 `scope`（模式 + bondId / characterId）与 `ui`（typing / pace / unread）。`lib/chat.ts` 是界面唯一的会话入口（`sendText` / `sendVoice` / `sendImage` / `sendCard` / `respond` / `peekMyPhone` / 四个 scope 构造器）；界面不再 import 引擎、记忆、约定识别。`lib/engine.ts` 退为门面（组历史 → completeChat → 拆气泡 → 剥暗号）。
  5. **prompt 文本仍集中在 `content/prompts.ts`**（D-017 精神不变），分段的模式与顺序声明在 `features/prompts.ts`；旧的四个 build 函数与 `bondedContextLines` 删除，改为共享段函数（introLine / voiceBlock / nowLine / birthdayLine / stageLine / squareSituationLines / outingMomentLine / outputFormatFor / lengthFor）。**D-017 修订**：玩法自己的一句话提示语（卡片进上下文的那句、发出时的舞台提示）随玩法文件，不再散在界面里。
  6. **安全网**：vitest（`npm test`）——14 份系统 prompt 快照（四模式 × 多情形 + 记事本，重构前从旧代码生成、重构后逐字一致）、回合管线集成测试（假供应商 + 真 store：记账 / 暗号落状态 / 心动满 offer / 失败露出原因 / 暗面路由绕过模型）、引擎工具函数单测。`tests/setup.ts` 把 react-native / expo-* 换成最小桩。改 prompt 的第一道验收 = 快照 diff。
  7. **类型**：`EngineReply.flags: Record<string, boolean>` 取代 `unlockPhone` / `openRedPacket`；`ChatCard.type` 放开为可扩展字符串；`Job.run` 返回值不限。
  8. **行为微调（有意）**：外出里心动满的 offer 现在也显示打字指示（与试聊一致）；通话与爽约走管线后，模型失败会在羁绊会话里落一条系统消息（D-069 口径），通话页仍照旧提示；爽约后 TA 的那一句现在也触发记忆提取。
  9. **手册**：`docs/ARCHITECTURE.md`（借了什么 / 目录 / 管线 / 玩法长什么样 / 菜谱 / 纪律 / 测试 / 下一轮）。
...
## D-090 · 2026-09-04 · 日历只记安排与纪念日：去掉「TA 自己稀疏长几条日程」的取样；TA 经历的事在发生之后进记事本或 X（Harper 提出）

- **背景**：Harper「日历会写角色未来的内容，比如一个礼拜之后的某天他路过一个琴行看见一个什么琴，这明显是不合理的。日历只用来记录安排和纪念日。类似内容可以在发生后，让他记到自己的备忘录或者朋友圈」。
- **原因**：D-020 时为了让日历「活」，日历 App 的 TA 层与 TA 手机里的日历都从角色台词库（`script.bonded`）取两句、按缔结日期 +5 / +16 天摆到未来。台词是过去式的生活片段，摆到未来就成了预知。
- **决策**：
  1. 两处取样删除；日历 App 的 `'him'` 层随之删除（图例、颜色）。日历现在只有：世界层（节假日）、关系层（交换联系方式纪念日、一百天、她的生日、TA 的生日）、约定（带时间的赴约）、用户层（手动日程）。TA 手机里的日历只剩和她的约定与 TA 的生日。
  2. 「TA 经历的事」已有归宿、不新增机制：TA 的记事本（`lib/his-notes.ts`，写「今天发生的事」）与 X 发帖（`lib/posts.ts`，「日常碎片、路上看见的东西」）都由调度器在事情「发生」的当下生成、按生成时间落款。
- **推翻**：修订 D-020 日历三层里「他自己也稀疏长几条日程」；修订 D-085 的 TA 手机日历口径。
- **影响文件**：`app/apps/calendar.tsx`、`components/his-phone.tsx`、`CLAUDE.md`。

## D-091 · 2026-09-04 · 通话页免提 / 听筒切换即时生效；聊天录音条改为整行显示（Harper 提出）

- **背景**：Harper「打电话免提和听筒切换不过来」「聊天里面的发语音要修改一下 UI 超框」。
- **原因与决策**：
...
## D-096 · 2026-09-06 · 已有账号直接登录：onboarding 第一步「已有账号？登录」；新手机登录以云端为准、不重走新手流（Harper 提出）

- **背景**：Harper「如果我有账号，但是没登录，就会重新走一遍逻辑，是不是不合适？一开始的第一步是不是应该有一个已有账号直接登录的按钮」。
- **查证**：此前登录入口只有两处——设置（到桌面之后才到得了）与首次入册的强制墙（D-062）。换了手机的老用户必须重走 onboarding → 滑卡 → 试聊到心动满 → 缔结，才第一次看到登录；而且**登录那一刻会把云端存档冲掉**：D-057 的对账规则「本机有未同步改动 → 本机覆盖云端（正在用的设备赢）」没有区分「这台手机同步过」和「新手机」——新装机一启动，后台任务与 onboarding 就把本机标脏，登录后 reconcile 直接 upload，旧 TA 们和故事被一份几乎空白的快照覆盖（设置里的「从云端恢复」此时也无从恢复）。
- **决策**：
  1. **onboarding 第一步（选语言）底部**加「已有账号？登录 · Sign in · ログイン」（语言还没选，三语并排；Supabase 未配置时不显示）→ `/auth?restore=1`：标题「欢迎回来」、可返回。
  2. **登录成功后先对账再走**（`app/auth.tsx` 的 `afterSignIn` → `reconcileNow`），三个入口（设置 / 强制墙 / 已有账号）共用：云端有存档且本机空 → 静默接回 → 落桌面（桌面按存档自己决定落点：还没 onboarding 的账号仍进 onboarding，轻提示「登录好了，先让 TA 们认识你吧」）；云端没存档 → 本机第一份传上去、照常继续。
  3. **同步规则修订**（`lib/sync.ts`）：对账决策抽成纯函数 `planReconcile`——**这台手机没和这个账号对过账**（`meta.lastSyncedAt === 0`，或 `meta.userId` 记的是别的账号）时本机不许覆盖云端：本机空（`localIsFresh`：没 onboarding，或既没羁绊也没自创角色；试聊记录属免费层、本就会过期，不算）→ 拉云端；本机已有关系 → 返回 `conflict`、什么都不动，由登录界面问她「接回云端的 / 用本机覆盖云端」（`resolveConflict`）。之后仍照 D-057（本机脏 → 推；云端更新 → 拉）。`meta.userId` 在每次成功上传 / 恢复时记下；D-096 之前没记账号的旧存档视为同一账号，老用户无感。`reconcileNow` 单飞（登录界面与 `onAuthChange` 同时触发只跑一份）。
  4. D-054 的「登录时弹恢复 / 覆盖二选一」以这种形式回来——只在两边都有关系的新设备上弹，其余情况自动。
- **推翻 / 修订**：修订 D-057 §3「正在用的设备赢」——加前提「这台手机和这个账号对过账」；修订 D-058 新手流（老用户可从第一步直接登录跳过）；部分恢复 D-054 §3 的二选一弹窗（限新设备冲突）。
- **影响文件**：`app/onboarding.tsx`、`app/auth.tsx`、`lib/sync.ts`、`lib/i18n.ts`、`tests/sync.test.ts`（新）、`CLAUDE.md`。
