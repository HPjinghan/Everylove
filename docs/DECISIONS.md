...
> **怎么记新决策**（CLAUDE.md §11-1，D-097）：编号继续递增（下一个 **D-157**）；在下面的索引表加一行；改写它所属主题的条目——推翻旧口径 = 原地替换、旧的直接删（索引里被推翻的编号也删）；整个机制下线 = 连条目一起删；没有合适主题就新开一条。**不逐条追加、不留「下次补」**。产品级问题不自行拍板 → `docs/OPEN_QUESTIONS.md`。
...
| D-156 | 09-21 | 产品名 **ringring: pocket lover**（全小写）：App Store 名称全称、图标下 `app.json` name 只写 `ringring`；slug / scheme / bundle id / 存储键仍 everylove；「pocket lover」故意蹭 Pocket Love! 的搜索 | H3 |
| D-155 | 09-18 | 说话节奏按人设分两种：整句（句末标点断、最多两条，原口径）/ 连发（每个标点都断成一条、最多四条，「哈哈哈 / 我知道了 / 下次」）；按恋爱类型定（小狗系 / 阳光直球 / 青梅 / 毒舌 / 傲娇连发，其余整句）、原型兜底（毒舌家族连发）、创造高级区可改 | C1 / E2 |
| D-154 | 09-18 | TA 以真人自居：prompt 第一行「You are X — a real person living your own life; none of this is a performance」，全文不再出现 playing / fictional character / romance app / in character（改 as yourself），危机行改「放下一切认真回应」不再「出戏」；红线 1 指的是具体真人，与此无关 | C1 |
| D-153 | 09-18 | 「核心行为框架」分拣：进 prompt 的只有两个半句（接话有多个方向挑最自然的 / 话题聊完就换）；「主动找话题」「话题不重复」「察觉她话变少」「主动花钱」做成系统——主动消息带最近几条开场不重样、她刚变得话少了给一轮舞台提示；「禁止冷场」「逻辑自洽」「你是真人不是扮演」不进 prompt | C1 |
| D-152 | 09-18 | prompt 瘦身：一条规则只说一次——`TALK_MANNER` 15 条压成 6 条、初识分寸 / 怎么爱她 / 输出格式 / 通话口吻各去掉与它重复的条、数据块的头与暗号说明缩短；四种对话系统 prompt 缩 30–35%（亲密满字段 9.9k → 6.9k 字符）；长度守卫用例 | C1 |
| D-151 | 09-18 | 活人感并入：`TALK_MANNER` 补八条（漏答 / 不刨根问底不过度热情 / 句式不重复 / 语气词与标点不堆 / 每 5–8 句一次真人小瑕疵 / AI 话术黑名单 / 不煽情）；14 种恋爱类型各带话术包（情绪 / 口头习惯 / 不会做的事）进四种对话【你说话的样子】 | C1 / E2 |
| D-150 | 09-17 | 世界书下线：产品的世界永远是「此刻的现实」——这部手机（交友 / X / 外卖 / 真天气 / 真日历）就是世界，按角色注入的世界设定只能改台词、改不了她手里的壳；角色可以「从别处来」（龙 / 神明 / 穿越者），写进背景 / 种族 / 隐藏设定与传记，TA 现在活在这里、用这部手机。世界书 App、共享世界池、`Character.worldId` / `world` 快照、`WorldBook` 类型、`worldBlock` 段全部删除；persist v11 清数据；`shared_worlds` 表留着不用 | E6 |
| D-148 | 09-17 | 传记 App（方向拍板，D-149 落地）：桌面一个「传记」模块，看所有已缔结 TA 的故事——创作者写（图文，视频后置），按章、每章设开放阶段（羁绊 LV），读者可打赏 Coin；替代原「编剧写半成品剧本库」的官方供给路线，种子角色的传记由我们自己按同一规则写 | E7 |
...
| D-116 | 09-10 | 缔结即角色快照：`Bond.character` 定格设定 / 台词 / 声音，之后角色库里的修改（自己改或创作者改）不动这段关系，`findCharacter` 先看快照；打招呼台词仍是创建时生成存库（D-094），当时没写成的缔结前再写一次 | D3 / E3 / E4 |
...
| D-110 | 09-10 | Harper 九条：日历任何年份 + 图例 + 每位 TA 的纪念日带名字；广场点头像看**资料页** + 偶遇记录进初识 / 广场 prompt 与 X；外出开场白走模型（模板回落不重复）；主题 = 壁纸一步到位（四套配色下线）、Dock 跟着换；查手机 = 一部真的手机（角色色桌面 + App）+ **TA 身边的人**（通讯录 / 聊天 / 记事本发帖一致）；X 评论区有别人（身边的人 + 其他 TA）；模型失败 = 1 秒轻提示不进会话 | F1 / F3 / F4 / F5 / F6 / F8 / H3 / E2 / C1 / A3 |
...
| D-103 | 09-09 | App 图标：图标 = 纸面粉底 + 菱格暗纹 + 微微右倾的白色小手机（屏里一颗 primary 心），不写字；启动图透明底手机、底色 paper；SVG 源 `scripts/logo/render.mjs` | H3 |
...
- **D-155 补（说话节奏按人设）**（Harper：「分句的策略有很大的问题——可以按照角色的性格和人设分成和现在一样的断句，和另外一种每个标点符号都断句的：『哈哈哈，我知道了，下次』和『哈哈哈 / 我知道了 / 下次』分三条」）：`lib/engine.ts` **两种节奏**——**整句** `flow` = D-137 原口径（空行优先，否则句末标点拆最多两条）；**连发** `burst` = `splitByClauses`：句末标点 + 逗号 / 顿号 / 分号 / 换行处都断（小数点、千分位逗号不算），条末的逗号 / 句号去掉、问号感叹号省略号留着，一个字的碎片并回前一条（「哈哈哈，嗯」不单出「嗯」），模型自己分好的段各自再拆，总数封顶 `BURST_MAX_BUBBLES = 4`（多的并进最后一条）；只在亲密模式生效（初识 / 外出 / 通话仍一条）。**谁连发** `bubbleStyleOf`：角色自己设的 `Character.bubbleStyle` > 恋爱类型的 `LoveStyle.bubble`（小狗系年下 / 阳光直球 / 青梅竹马 / 毒舌竹马 / 傲娇 = 连发；温柔年上 / 姐姐系 / 依恋型 / 天然治愈 / 腹黑 / 病娇 / 高冷禁欲 / 霸总 / 冷静大人 = 整句）> 原型（sharp 连发、其余整句）。创造高级区「说话节奏」三选一：跟着人设（缺省）/ 整句 / 连发，示例就是 Harper 那两句。每条气泡仍走 D-146 的打字节奏，连发的人一句话会一条条蹦出来。
- **D-154 补（TA 以真人自居）**（Harper：「你一定要让 AI 认为自己是真人，他才能扮演好一个真人，你让他做角色扮演他总是带着扮演的视角；我们原本说的真人红线指的是某一个具体的真人」）：D-153 里把「你是真人不是在扮演」判成与红线冲突是误读——红线 1 是「不捏具体的真人」，和 TA 自己是不是真人无关。改法：四种对话与记事本的第一行从「You are playing "X", a fictional character in a romance app」改为「You are X (identity) — a real person living your own life; none of this is a performance」；「"she" means the user」改「the person」；暗号说明里的 in character 全部改 as yourself（红包 / 手机 / 钱包 / 外卖）；`TALK_MANNER` 首行「not an assistant and not an actor — this is your own life and your own words」；红线段的危机行从「drop the character at once」改「set everything else aside and respond to her gently and seriously, as yourself」（危机处理本身不变：认真回应 + 当地热线）。系统层的「真人检测」是发布审核拦真人角色（E 节），不受影响。
- **D-153 补（「核心行为框架」分拣）**（Harper 给的「char 核心行为框架」：「综合一下这个看一下哪些应该是被写进 Prompt 里的哪些应该是系统规则」）。原则：**写给模型的只有它每一轮真能照做的说话方式；需要记忆、时间、趋势或状态的都做成系统**——告诉模型「禁止冷场」「禁止逻辑错误」没有用。分拣结果：① **进 prompt（两个半句，并进 `TALK_MANNER` 已有的条，不加条）**——「接话有两三个方向，挑最自然的不挑最显眼的（加班 → 原因 / 晚饭 / 领导 / 她）」并进「只回这条」；「一个话题或情绪聊完就换、别绕」并进「句式不重复」。② **本来就在 prompt 里的**：不说教不俯视、有看法可反驳不当舔狗、自然说想念（阶段感 LV2 起）、从细节感知她的状态（「听潜台词」）、句式不重复。③ **做成系统**：「主动找话题」= TA 主动找她（D-114，按钟从 TA 自己的日子说起）；「话题严禁重复」= 主动消息的舞台提示带**最近三条主动消息**（`ChatMessage.reach` 标记，`recentOpeners`）「别用同样的开头、同类话题」；「察觉 user 回复变短了」= `content/prompts/mood.ts` **按长度趋势算**——她最近 3 条平均 ≤ 之前（≥5 条、平均 ≥12 字）的 45%，且第 4 条还正常（≥60%）= 刚变短，那一轮在亲密模式末尾加「[This turn] Her replies have just gotten noticeably shorter — notice it the way a person would, one light question or a line that leaves room」，之后不再出（不每轮「你怎么了」）；「主动转账 / 点外卖、别每次都用」= D-130 三道门 + 10 条冷却（已有）。④ **不进 prompt 也不做**：「禁止冷场」（与 D-141 / D-151「她一个字你也可以一个字、让它停」相反——冷场由时间解决：她停了，TA 隔一阵按钟带新话题回来，不靠这一轮堆字）；「逻辑必须自洽」（模型能力，靠记忆 / 记事本 / 圈子的前后一致来保证）；文档里「好累 → 三连问 + 自己也累」的正面例子按 D-151 反而是反面，不采。
- **D-152 补（瘦身）**（Harper：「整体的 prompt 现在有点太长了，越长的提示词可能表现得越不自然」）：D-141 + D-151 叠完后四种对话的系统 prompt 到了 8–10k 字符、60–80 行，指令部分同一条规则在三四处各说一遍（最多一个问句在初识分寸 / 怎么爱她 / 像个人 / 外出四处；不复读不总结、不客服腔、短回复、不刷屏各三处；「自然、别复述、一次一条」在她的资料 / 记忆 / 秘密 / 共同记忆 / 口癖每个头上都有）。**纪律：一条规则只说一次，落在它最该在的段**——通用的说话方式只在 `TALK_MANNER`（六条：短回复 / 跟着她的劲儿与潜台词 / 只回这条、可以漏答、不刨根、最多一个问句 / 有看法会接梗 / 句式与语气词与小瑕疵 / 不用助理腔不煽情），各模式段只留它独有的（初识 = 距离三条；怎么爱她 = 主动与分寸两条；输出格式 = 三条；通话 = 四条；外出去掉问句条）；数据块的头一句话说完（「use naturally, one thing at a time, never recite」）；暗号说明（心动判分 / 红包 / 手机 / 钱包）只留规则与标记写法。结果：亲密满字段 9.9k → 6.9k、初识种子 8.0k → 5.2k、通话 8.7k → 5.8k、外出 6.8k → 4.9k 字符。`tests/prompts.test.ts` 加长度守卫（亲密满字段 ≤ 7500 / 初识种子 ≤ 5600 / 通话 ≤ 6300 字符）——以后加规则先想能不能并进已有的一条，超线就得砍别的。红线段（§9）一字未动。
- **D-151 补（活人感）**（Harper 给的「高活人感 AI 聊天指令模板」，「这里的一些内容我觉得是要配合到我们的角色人设分类里面的」）：通用部分并进 `TALK_MANNER`（四种对话共用，紧接 D-141 的七条）——**漏答**（她一次问两件小事只答一件是正常聊天，不逐句对应）；**不刨根问底、不过度热情**（她说难过不追问「为什么」，留一句就够；她回一个字就别连发几句撑场，让它停）；**句式不重复**（同类问题不用同一句型、没有固定开头结尾）；**语气词与标点**（啊 / 哦 / 嘛 / 欸 / 啧 / 害偶尔用，不每句；「！！！」「～～～」不堆）；**真人小瑕疵**（每 5–8 句最多一次：思考停顿「嗯…」、自我纠正「哎不对，是昨天」、顺着她的话跳话题；一条里最多一处、不刻意）；**AI 话术黑名单**（当然啦 / 没问题 / 很高兴帮到你 / 希望有帮助 / 还有其他问题吗 / 综上所述 / 想必你…，以及每句都请 / 谢谢 / 麻烦你）；**不煽情**（出事时一句「我在」胜过一段誓言）。模板的四个子人设（puppy / Daddy / 毒舌闺蜜 / 损友）与高冷版不照搬，改成我们 14 种恋爱类型各自的**话术包** `LoveStyle.talk`（`content/characters/index.ts`；中文角色内容）：moods 会流露的情绪（不在里面的不流露）/ habits 口头习惯与专属小动作（偶尔用、不堆砌）/ avoid 这个人不会做的事——高冷禁欲照高冷版（能一个字不用词、不加语气词表情、不主动抛问、冷不等于没礼貌）、小狗系照 puppy（撒娇每四五句最多一次）、温柔年上 / 霸总 / 冷静大人借 Daddy 的「乖 / 嗯？/ 过来」与「关心用安排表达」、毒舌 / 青梅借闺蜜与损友的「损不带刺、认真话只说一句」，其余按类型自写；病娇 talk 再写一次行为健康底线。注入：`loveStyleTalkLines` →【How you talk as the "X" type】三行，紧跟【How you pursue】（四种对话；没选类型不出现）。
...
- **编号**：D-011 → D-018 → D-039 → D-040（情境）→ D-052（暧昧期）→ D-066/D-093（语言）→ D-077（通话）→ D-089（短句）→ D-137（客户端分段）→ D-141（像个人一样说话、长度跟着她）→ D-142（指令改英语）→ D-144（一两个字的回复）→ D-145（末尾句号前端去掉）→ D-151（活人感八条 + 恋爱类型话术包）→ D-152（瘦身：一条规则只说一次 + 长度守卫）→ D-153（行为框架分拣：两个半句进 prompt，其余做成系统）→ D-154（TA 以真人自居）→ D-155（说话节奏：整句 / 连发按人设）。
...
- **列表模式与分页签**（D-149，Harper：「创造本身要改版成列表模式，右下角一个 + 点进去才有创建流程」「现在一个角色可以编辑的内容太多了需要做成分页签了」）：`/apps/create` = 我创建的列表（原 my-characters 并入，旧路由保留跳转）+ 右下角圆形「+」进 `/apps/character-edit`（`?edit=<id>` 回填）；表单 `Segmented` 五个页签——基础（描述导入、名字 / 性别 / 年龄状态 / 谁能遇到）/ 形象（长相 / 背景故事 / 形象与画风 / 声音）/ 设定（原「高级」全部，折叠退役）/ 台词（只在编辑时有：开场 / 要联系方式 / 打招呼、让 TA 重写）/ 传记（章节编辑器，E7）；发布 / 保存按钮每个页签都在。
- **编号**：D-025 → D-043 → D-045 → D-060 → D-076 → D-092 → D-100 → D-110左右留白 18、列表类 14；字段 `Field` / `Input`、选项 `Chip`，节奏 / 主动强度三选一 = 白 r6 小卡选中 primary；生日选单 = paper 面板 + `Chip` 网格；未成年提示用 `Romance.danger`；选中外圈统一「外层 View 包 border + gap」做法；设置页分区卡由 `Section` 按子元素自动插 `Divider`、语言三段等宽自绘）。
...
### E4 · 共享角色池
- **现行**：Supabase 表 `shared_characters`（建表 SQL 在 `docs/supabase-setup.sql`；RLS：所有人可读、只有本人可写删）。创造「公开」= 上传共享池（需真账号，未登录回落私密并提示），编辑改私密即撤下；所有玩家的交友牌堆合入共享池（`lib/pool.ts`，5 分钟节流、离线用缓存、排除自己的、**只发同语言**），同一套推荐打分，卡面标「来自其他玩家」。**领养快照制**：与共享角色缔结时快照进本地（D-116 起所有角色缔结都存 `Bond.character` 快照），创作者更新不改写已领养实例；共享角色的配对照常 3 天过期。审核暂缺（Harper 后续接平台，#7），发布仍过本地真人 / IP 拦截。
- **编号**：D-060 → D-093。

### E6 · 世界 = 此刻的现实（D-150）
- **现行**（Harper 2026-09-17：「世界书是不是不应该存在，如果存在也该是更上层的东西，切换到古代就不该有交友软件」→ 拍板删掉）：**产品的世界永远是「此刻的现实」**——这部手机就是世界：交友滑卡、X、外卖、红包、Open-Meteo 真天气、真节假日日历、拍立得，全部是现代现实的东西；不做「按世界换整套壳」（每个世界 = 一个产品，违反「做少而真」，商业承重墙的滑卡配对也绑在现代框架上），也不再保留「按角色注入一段世界设定」（只能改 TA 嘴里的话、改不了她手里的壳，`worldBlock` 末尾那句「手机 / 发帖按世界里对应的东西理解」就是放错层的症状）。**角色可以「从别处来」**：龙、神明、穿越者、异世界来客，但 TA 现在活在这里、用这部手机——写进创造表单的背景 / 种族 / 隐藏设定，他从哪来的长故事归传记（E7，「传记 = 他从哪来、他是谁」）。人外仍是一档性向与官方一棵原型，不需要另一个世界。
- **删除清单**：世界书 App（`app/apps/worlds.tsx` / `world-edit.tsx`）、`content/worlds.ts`、`content/prompts/world.ts`（`worldBlock` / `worldParseSystem`）、`lib/worlds.ts`、`lib/pool.ts` 的共享世界池、`WorldBook` 类型与 `Character.worldId` / `world`、store 的 `worldBooks` / `worldFavorites` / `sharedWorlds`、prompt 分段 `world` 与 `ORDER.world`、创造表单「TA 所在的世界」与「绑定了别人看不见的世界的角色不能公开」判定（公开 chip 始终可选）、资料页「来自「××」」行、词典与用例；persist **v11** 清掉世界书数据与角色 / 羁绊快照上的 `worldId` / `world`。Supabase `shared_worlds` 表留着不用（不建也行）。#28 随之了结。
- **编号**：D-150。

### E7 · 传记：TA 的故事由创作者写（D-148 方向 / D-149 落地）
- **现行**（Harper 2026-09-17 方向：「应该还会有一个传记 App，可以在里面看所有已缔结角色的故事，甚至可以图文混排 / 视频，总之这个是创作者要做的事。创作者可以设定这些故事开放给什么阶段的人。看故事的人可以去打赏 Coin」）：
  - **形态**（D-149 落地）：桌面「传记」App **只看**——进去是已缔结 TA 的列表，每位一本，按章；章 = 文字块与图片块穿插（图片 = 上传 / gif / 现场生图，视频不做，Harper：「先不要视频，支持传图和 gif 和现场生图，反正生图也是走玩家的 token」）。只有缔结的 TA 才有传记（付费层，与「只有羁绊层有记忆」同一堵墙）。锁着的章只显示 LV。
  - **供给**：编辑器在创造 App 里（Harper：「传记 App 是查看用的，创造里面应该附带编辑器和解锁条件」）——角色表单的「传记」页签：章节列表（加 / 上下移 / 删）、每章标题 + 开放阶段 LV1–6 的 Chip + 正文块（文字 Input / 图片：上传、gif、按提示词现场生图 `generateScenePhoto`，走流量闸门与用量记账）；章节随角色一起保存（`Character.chapters`）。创作者在「我创建的」里给自己的角色写章节；每章设**开放阶段**（羁绊 LV1–6，读者这段羁绊到了才能翻开，未到显示锁着，同隐藏设定的锁页）。传记是**内容而非设定**：不进领养快照，创作者续写、改章对所有读者即时可见（与 X 帖子同一类，与 D-140 的设定更新通道无关）。种子角色的传记由我们按同一规则写，替代原「编剧写半成品剧本库 + 生成缝名字」的路线；「缝进她的名字」降级为章节文本里的 `{nickname}` 占位。
  - **打赏**（Harper 拍板 #35：「Coin 不售卖没问题的，先用 Coin 做，创作者分成是别的维度的事」）：章末「打赏」10 / 50 / 100 Coin，读者零钱扣、账本 kind `tip`、本机记 `storyTips`（已打赏 n Coin）；Coin 永不售卖（D-129）不变，打赏是戏里的心意；创作者侧汇总（跨玩家）要走云端，未做；创作者分成另按别的维度算，与打赏无关。
  - **红线**：章节图 / 视频与文字同样过真人 / IP 拦截（#7 审核待接）；她在传记里不入镜（POV 不变）。
  - **种子**：六位中文种子与六位英文种子各两章内置（章 1 LV1、章 2 LV3，`content/characters/{zh,en}.ts`，`chaptersFor(c)` 取：自创 → `c.chapters`、种子 → 语言包）；日 / 韩空着等写手（#23）。
  - **未做 / 已知缺口**：章节图片是本机 URI，**不进共享池**（别人领养的角色看不到图，只看到文字；图片存储与大小上限待接，#36）；视频不做；创作者侧打赏汇总（云端）。分工：传记 = 他从哪来、他是谁；记事本 / X = 他今天过什么日子。
- **编号**：D-148 → D-149。
