...
> **怎么记新决策**（CLAUDE.md §11-1，D-097）：编号继续递增（下一个 **D-099**）；在下面的索引表加一行；改写它所属主题的条目——推翻旧口径 = 原地替换 + 「曾经」加一行；没有合适主题就新开一条。**不逐条追加、不留「下次补」**。产品级问题不自行拍板 → `docs/OPEN_QUESTIONS.md`。
...
| D-098 | 09-07 | TA 的记事本写自己的生活：专用装配、七成不写她、接着上几条写 | F6 |
