...
- **编号**：D-027 → D-030 → D-073 → D-074 → D-081 → D-084 → D-091 → D-100 → D-146（另：Fredoka 只在字符串为纯 ASCII 时启用——「昨天 / n 天前」这类时间标签回落系统字体；列表无未读时留 19 高占位对齐时间列；「TA 的主页」三个动作先收起主页、在 sheet dismiss 后再执行，一次只开一个 Modal；主页纪念日只显示 M/D；Fredoka 数字与中文并排 Text 而不嵌套，避免继承 fontWeight 回落）。
