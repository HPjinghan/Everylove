# 决策日志存档（2026-08-13 ～ 2026-09-06，D-001 ～ D-096）——冻结，只供考古

> 2026-09-06（D-097）起，现行口径以 `docs/DECISIONS.md`（按主题合并的总账）为准；本文件是合并前的逐条原文，**不再维护、不再追加**。查某个编号当时的完整理由与影响文件，到这里按编号搜。
> 注意：本文件里 D-087 与 D-088 各出现两次（撞号），总账里以 D-087a/b、D-088a/b 区分。
>
> 原格式：编号 / 日期 / 决策 / 理由 / 是否推翻旧决策及其编号 / 影响的文件
