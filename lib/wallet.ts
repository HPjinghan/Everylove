 ...
 * - 拟真游戏币，¥ 显示；永不售卖、不可提现、不随订阅变化、不涨亲密度（钱买不到爱）。
 * - 她的钱包：日签 App 每天抽一次水晶球，按运势给 ¥50–500；红包 / 外卖进出都记账（store.wallet）。
 * - TA 的钱包（Bond.wallet）：缔结 ¥2000 起；周薪按人设由模型估一次（写不成按关键词兜底），每周到账；TA 发红包 / 点外卖从这里扣。
