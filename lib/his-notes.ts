/**
 * TA 的记事本调度器（D-085）：TA 按 MBTI 的频率往自己的记事本里写心事（她查手机时看得到）。
 * - 频率：I 比 E 更爱写本子（与发帖相反），NF 最多、ST 最少；无 MBTI 默认 1 条/天；间隔 ±35% 抖动
 * - 与发帖同机制：启动 / 回前台 / 打开 TA 的手机时补写；一条都没有时立刻写第一条（本子不空着）；错过再久只补 1 条
 * - 内容由引擎按亲密模式背景生成（prompts §9），AI 不可用 / 失败 = 这次不写
 */

import { buildHisNoteSystem, HIS_NOTE_USER } from '@/content/prompts';
...
async function generateNote(character: Character, bond: Bond): Promise<string | null> {
  ...
  if (!ctx) return null;
  try {
    const raw = await completeText(buildHisNoteSystem(ctx), HIS_NOTE_USER, 240);
