...
12. **设置**：账号 · 云端（登录 / 立即备份 / 从云端恢复 / 退出 / 删除云端数据）、语言、我的身份、主题（配色 4 套 + 纸面默认；壁纸 = 纸面 + 5 款换色、暗纹常在）、订阅计划（试装模拟；槽位超额时数值标 accent 并说明不能再新增）、素材开关、我的创作、开发者（只读 AI 引擎与取路、记忆库查看、立绘重画、重置）。
